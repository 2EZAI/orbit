...
export function UnifiedDetailsSheet({
  ...
  const locationIdForEvents = !isEventData(data, isEvent) ? data.id : null;
  ...
  const handleShare = async () => {
    const currentData = detailData || data;
    const isEventType = isEventData(currentData, isEvent);
  ...
  const handleCreateEvent = () => {
    if (isEventData(data, isEvent)) return;
  ...
  const handleJoinEvent = async () => {
    if (!isEventData(detailData || data, isEvent)) return;
  ...
  useEffect(() => {
    const eventName = isEventData(data, isEvent)
      ? "refreshEventDetail"
      ...
      <View style={{ flex: 1 }}>
        ...
        index === currentIndex ? 'bg-yellow-400' : 'bg-white/70'
            ...
            <View className="px-6 pt-6">
                    ...
                    (currentData as any).category.prompts.length > 0 && (
                      ...
                      >
                        <View className="flex-row justify-between items-center mb-3">
                          ...
                          {(currentData as any).category.prompts.length > 3 && (
                            <View className="flex-row gap-2">
                              {/* Shuffle Button */}
                              <TouchableOpacity
              ...
              {/* Photo Gallery */}
              {currentData?.image_urls && currentData.image_urls.length > 1 && (
                <View className="mb-6">
                  <Text
                    className="mb-3 text-lg font-bold"
                    style={{ color: theme.colors.text }}
                      ...
                      {similarItems.map((item: any, index: number) => (
                        ...
                        >
                          ...
                          <View className="p-3">
                            ...
                            {isEventData(item) && item.attendees?.count > 0 && (
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    Use the shuffle button to discover new activities each time
